...
**Из сложного была обработка ошибки, которая возникала, когда предпринималась попытка удалить читателя, у которого либо была книга на руках, либо он в целом упоминался в выдаче. Реализованное решение:**
  - Сделать многоуровневую проверку перед удалением (проверка активных задач).
  - Удаленить все записи о выдача книг (каскадное удаление), а затем удалить самого читателя и связанные персональные данные.

#### BookService
- Для работы использует только свой репозиторий.

- **Метод create()**:
  - Проверяет, есть ли уже книга с таким ISBN через `exists_by_isbn()`.
  - Проверяет, существует ли автор через `author_exists()`.
  - Если проверки пройдены - создает книгу через репозиторий.

- **Метод update()**:
  - Проверяет, существует ли книга.
  - Если обновляется ISBN - проверяет его уникальность среди других книг.
  - Выполняет обновление через репозиторий.

- **Метод delete()**:
  - Проверяет, существует ли книга.
  -  Проверяет, нет ли активных выдач этой книги, чтобы информация о выдаче не пропала.
  -  Если проверки пройдены - удаляет через репозиторий.

- **Метод get_by_id()**:
  - Пытается получить книгу через репозиторий. Бросает исключение, если не найдет
  - Возвращает найденную книгу по id.

- **Метод get_all()**:
  - Получает список всех книг без дополнительных бизнес-проверок.
...
- **Метод borrow_book**:
  - Проверяет, существует ли читатель с указанным id (через вызов reader_exists в `ReaderRepository`).
  - Проверяет, есть ли в библиотеке экземпляры книги для выдачи (через is_book_available в `BookRepository`).
  -  Проверяет, может ли читатель взять более 3 книг одновременно (get_active_borrowings из `BorrowedBookRepository`).
  - После успешного прохождения всех проверок сервис уменьшает количество доступных экземпляров книги через decrease_book_copies и создает новую запись о выдаче с помощью метода create в `BorrowedBookRepository`.

- **Метод return_book**:
  -. Ищет запись о выдаче книги указанному читателю, которая еще не помечена как возвращенная. Дальнейшая работа прерывается, если книга уже возвращена или активной выдачи не найдено.
  -. Обновляет данные (количество доступных экземпляров увеличивается на 1 через `increase_book_copies`, запись о выдаче, устанавливая дату возврата с помощью `mark_returned`).
...
- **Метод create**:
  - Перед созданием записи проверяет уникальность email через метод `get_by_email()` репозитория. Если email уже занят, генерируется ошибка.
  - Пароль хешируется с помощью `PasswordSecurity`, после чего создается объект `LibrarianRepoCreate` с данными личности и хешированным паролем.
  - Финальное сохранение выполняется через метод `create()` репозитория.

- **Метод update**:
  - Проверяется существование библиотекаря через `get_by_id()`.
  - Если в обновляемых данных меняется email, выполняется дополнительная проверка его уникальности среди других записей.
  - Для обновления используется `LibrarianRepoUpdate` схема, которая содержит только те поля, которые можно изменять (на данный момент все, кроме пароля. Пароль меняесят отдельно).
...
- **Метод delete**:
-   Проверяет существование читателя.
-   Проверяет отсутствие не возвращенных книг (через репозиторий).
-   При наличии активных выдач возвращает понятное сообщение об ошибке.
-   Удаляет все связанные записи (выдачи, персональные данные).
...
- При логине (/auth/login) система проверяет email и пароль через `AuthService.authenticate()` и Создает токен с email в поле sub через `create_access_token()`.
- Верификация токенов происходит через `AuthService.verify_token()` с проверкой: корректности подписи, наличия обязательного поля `sub`, срока действия токена.
...
- Все POST, PUT, PATCH, DELETE операции. Но с одним но: библиотекаря могут изменять и удалять только свои данные.
- Чувствительные операции: смена пароля и просмотр персональных данных.

### Особенности реализации:
- Пароли хранятся как SecretStr для предотвращения случайного логирования.
-  Все проверки выполняются на уровне сервисов, а не репозиториев.
- Каскадное удаление связей при удалении пользователей.
- Автоматический откат транзакций при ошибках.

### Творческая часть
#### Система рейтинга книг на основе их популярности:
- Добавить в модель Book поле borrow_count для подсчета количества выдач, инкрементировать счетчик при каждой успешной выдаче.
- Создать эндпоинт с количеством возвращаемых книг и фильтром по периоду (неделя/месяц и тд).
#### Учесть, что авторов может быть несколько, и создать связь Many-to-Many между книгами и авторами. Для этого бы потребовалось:
- Создать модель Author и промежуточную таблицу book_authors.
- Модифицировать все связанные эндпоинты.
